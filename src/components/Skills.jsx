...
const Skills = () => {
  return (
    <section id="skills" className="py-24 px-6 relative bg-white dark:bg-gray-950 overflow-hidden">
      {/* Creative Background Ambient Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-tr from-blue-500/5 via-purple-500/5 to-transparent rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {skillCategories.map((category) => {
            ...
            return (
              <div
                key={category.title}
                className="group relative bg-gray-50/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-2xl p-7 border border-gray-200/80 dark:border-gray-800/80 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between overflow-hidden"
              >
                ...
                <div>
                  {/* Card Header */}
                  <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-200/60 dark:border-gray-800">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center border shadow-xs ${category.iconBg}`}
                      >
                    ...
                    <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border border-gray-200/80 dark:border-gray-700/80 shadow-2xs">
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {category.skills.map((skill) => (
                      <span
                        key={skill}
                        className={`group/item inline-flex items-center gap-2 px-3 py-1.5 text-xs md:text-sm font-medium rounded-lg
